...
- **Database credentials** — `SQLITE_DATABASE_PATH` (required); `REDIS_URL` (only if credits system is enabled)
...
When `fly launch` detects the existing `fly.toml`, it will ask if you want to use it — say **yes**. The `--no-deploy` flag creates the app on Fly.io without deploying yet — you still need to configure secrets and (optionally) Redis first.

> **Important:** Before launching, open `fly.toml` and change the `app` name from `qiforge` to your oracle's name (e.g., `my-oracle`). This name becomes your Fly.io subdomain (`my-oracle.fly.dev`).
